...
pub async fn run(mut args: Vec<String>) -> anyhow::Result<()> {
    ...
    let published_report_uri =
        take_optional_flag_value(&mut args, "--published-report-uri")?.unwrap_or(report.clone());
    let actor = take_flag_value(&mut args, "--actor")?;
    let notes = take_flag_value(&mut args, "--notes")?;
    if !args.is_empty() {
        anyhow::bail!("unexpected arguments: {}", args.join(" "));
    }
    let response = crate::submit_worker_data_pipeline_execution_report_with_published_uri(
        &api_url,
        &api_key,
        &report,
        &actor,
