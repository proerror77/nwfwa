    ...
    match args.remove(0).as_str() {
        ...
        "serve-mlops-alert-router" => {
            ...
            let scheduler_report = take_optional_flag_value(&mut args, "--scheduler-report-uri")?
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "missing --scheduler-report-uri or FWA_MLOPS_SCHEDULER_REPORT_URI"
                    )
                })?;
            ...
            let notes = take_optional_flag_value(&mut args, "--notes")?
                ...
                .unwrap_or_else(|| {
                    "Alertmanager webhook converted by MLOps alert-router adapter.".into()
                });
            if !args.is_empty() {
                anyhow::bail!("unexpected arguments: {}", args.join(" "));
            }
            worker::serve_mlops_alert_router(worker::MlopsAlertRouterConfig {
                bind_addr,
        ...
        "promote-approved-model-version" => {
            let api_url = take_flag_value(&mut args, "--api-url")?;
            let api_key = take_flag_value(&mut args, "--api-key")?;
            let model_key = take_flag_value(&mut args, "--model-key")?;
            let model_version = take_flag_value(&mut args, "--model-version")?;
            ...
            let result = worker::promote_approved_model_version(
                &api_url,
                &api_key,
                &model_key,
                &model_version,
