...
fn extract_evidence_sufficiency(ctx: &InvestigationContext) -> EvidenceSufficiency {
    ...
    EvidenceSufficiency { present: vec![], missing: vec![] }
}
...
fn supplement_request_button() -> Html {
    ...
    if *sent {
        ...
        html! {
            <button
                style="background:#1f6feb;color:#e6edf3;border:none;border-radius:6px;padding:8px 16px;font-size:0.85rem;cursor:pointer;font-weight:600;"
                onclick={on_click}
            >
                {"发补件请求"}
            </button>
...
pub(crate) fn layer_document_completeness(ctx: &InvestigationContext) -> Html {
    ...
    let lines = vec![
        BillingLine { code: "99213", desc: "门诊复诊",  claimed: base_amount * 0.18, peer_avg: peer_avg * 0.20 },
        BillingLine { code: "93000", desc: "心电图",    claimed: base_amount * 0.07, peer_avg: peer_avg * 0.06 },
        BillingLine { code: "85025", desc: "血常规检查", claimed: base_amount * 0.05, peer_avg: peer_avg * 0.05 },
        BillingLine { code: "99232", desc: "住院随诊",  claimed: base_amount * 0.70, peer_avg: peer_avg * 0.69 },
    ];
    ...
    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div class="evidence-card-header" style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"① 资料完整性 & 金额合理性"}</h4>
                { status_badge }
            </div>

            // Checklist
            <div style="margin-bottom:14px;">
                <p style="margin:0 0 8px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"最低资料要求"}</p>
                <div style="display:flex;flex-direction:column;gap:6px;">
                    { for checklist.iter().map(|(item, ok)| {
                        let (icon, color) = if *ok { ("✓", "#3fb950") } else { ("✗", "#f85149") };
                        let label = if *ok { "已提供" } else { "缺失" };
                        html! {
                            <div style="display:flex;align-items:center;gap:8px;">
                                <span style={format!("font-size:0.9rem;font-weight:700;color:{color};")}>{icon}</span>
                                <span style="font-size:0.85rem;color:#e6edf3;flex:1;">{*item}</span>
                    ...
                    { for suf.missing.iter().filter(|m| {
                        ...
                        html! {
                            <div style="display:flex;align-items:center;gap:8px;">
                                <span style="font-size:0.9rem;font-weight:700;color:#f85149;">{"✗"}</span>
                                <span style="font-size:0.85rem;color:#e6edf3;flex:1;">{m.as_str()}</span>
                                <span style="font-size:0.75rem;color:#f85149;">{"缺失"}</span>
                            </div>
                        }
                    }) }
                </div>
            ...
            <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;margin-bottom:14px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                    <p style="margin:0;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"金额合理性"}</p>
                    <span style="font-size:0.7rem;background:#21262d;color:#8b949e;border:1px solid #30363d;border-radius:4px;padding:1px 6px;">{"模拟数据"}</span>
                </div>

                // Billing line grid
                <div style="display:grid;grid-template-columns:5rem 1fr 5rem 5rem;gap:4px 12px;font-size:0.8rem;margin-bottom:12px;">
                    <span style="color:#8b949e;font-weight:600;">{"代码"}</span>
                    <span style="color:#8b949e;font-weight:600;">{"项目"}</span>
                    <span style="color:#8b949e;font-weight:600;text-align:right;">{"申报金额"}</span>
                    <span style="color:#8b949e;font-weight:600;text-align:right;">{"同行均值"}</span>
                    { for lines.iter().map(|line| {
                        let ratio     = line.claimed / line.peer_avg;
                        let val_color = if ratio > 1.3 { "#f85149" } else if ratio > 1.1 { "#d29922" } else { "#e6edf3" };
                        html! {
                            <>
                            <span style="color:#8b949e;">{line.code}</span>
                            <span style="color:#e6edf3;">{line.desc}</span>
                            <span style={format!("color:{val_color};text-align:right;")}>{format!("¥{:.0}", line.claimed)}</span>
                            <span style="color:#8b949e;text-align:right;">{format!("¥{:.0}", line.peer_avg)}</span>
                            </>
                        }
                    }) }
                </div>
                ...
                <div style="margin-bottom:4px;">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                        <span style="font-size:0.75rem;color:#8b949e;width:5rem;">{"申报合计"}</span>
                        <div style="flex:1;background:#21262d;border-radius:3px;height:10px;overflow:hidden;">
                            <div style={format!("width:{claimed_bar}%;height:100%;background:{};border-radius:3px;",
                                if over_threshold { "#f85149" } else { "#1f6feb" })}></div>
                        </div>
                        <span style="font-size:0.75rem;color:#e6edf3;width:5rem;text-align:right;">{format!("¥{:.0}", total_claimed)}</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="font-size:0.75rem;color:#8b949e;width:5rem;">{"同行均值"}</span>
                        <div style="flex:1;background:#21262d;border-radius:3px;height:10px;overflow:hidden;">
                            <div style={format!("width:{peer_bar}%;height:100%;background:#3fb950;border-radius:3px;")}></div>
                        </div>
                        <span style="font-size:0.75rem;color:#8b949e;width:5rem;text-align:right;">{format!("¥{:.0}", total_peer)}</span>
                    </div>
                </div>

                { if over_threshold {
                    html! {
                        <div style="margin-top:8px;padding:6px 10px;background:#3d1a1a;border:1px solid #f85149;border-radius:4px;font-size:0.8rem;color:#f85149;">
                            {"⚠ 申报总额超出同行均值 25%，存在异常"}
                        </div>
                    }
                } else { html! {} } }
            ...
            if let Some(arr) = ev.payload.get("alerts").and_then(|v| v.as_array()) {
                return arr
                    ...
                    .filter_map(|a| {
                        a.as_str()
                            .map(str::to_string)
                            .or_else(|| a.get("message").and_then(|m| m.as_str()).map(str::to_string))
...
fn evidence_ref_type_label(r: &str) -> (&'static str, &'static str) {
    if r.starts_with("rule_runs:")    { ("规则命中", "#d29922") }
    else if r.starts_with("model_scores:") { ("模型评分", "#58a6ff") }
    else if r.starts_with("audit_events:") { ("审计事件", "#8b949e") }
    else if r.starts_with("claims:")       { ("理赔记录", "#3fb950") }
    else                                   { ("证据引用", "#8b949e") }
}
    ...
    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"② 风险信号"}</h4>
            ...
            { if !reasons.is_empty() {
                html! {
                    <div style="margin-bottom:14px;">
                        <p style="margin:0 0 8px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"标记原因"}</p>
                        <div style="display:flex;flex-direction:column;gap:5px;">
                            { for reasons.iter().enumerate().map(|(i, r)| {
                                let bg     = if i == 0 { "#3d1a1a" } else { "#21262d" };
                                let border = if i == 0 { "#f85149" } else { "#30363d" };
                                html! {
                                    <div style={format!("background:{bg};border-left:3px solid {border};border-radius:0 4px 4px 0;padding:6px 10px;font-size:0.83rem;color:#e6edf3;")}>
                                        {*r}
                                    </div>
                                }
                            }) }
            ...
            { if !ev_ref_chips.is_empty() {
                html! {
                    <div style="margin-bottom:14px;">
                        <p style="margin:0 0 8px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"证据类型"}</p>
                        <div style="display:flex;flex-wrap:wrap;gap:6px;">
                            { for ev_ref_chips.iter().map(|(r, kind, color)| {
                                html! {
                                    <span style={format!("background:#21262d;border:1px solid {color};border-radius:4px;padding:2px 8px;font-size:0.75rem;color:{color};")}
                                          title={r.clone()}>
                                        {*kind}
                                    </span>
                                }
            ...
            { if !alerts.is_empty() {
                html! {
                    <div style="margin-bottom:14px;">
                        <p style="margin:0 0 8px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"规则告警"}</p>
                        <div style="display:flex;flex-direction:column;gap:5px;">
                            { for alerts.iter().map(|a| {
                                html! {
                                    <div style="background:#3d2000;border-left:3px solid #d29922;border-radius:0 4px 4px 0;padding:6px 10px;font-size:0.82rem;color:#e6edf3;">
                                        {a.as_str()}
                                    </div>
                                }
                            }) }
            ...
            { if has_scores {
                html! {
                    <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;">
                        <p style="margin:0 0 10px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"7层评分分解"}</p>
                        <div style="display:flex;flex-direction:column;gap:7px;">
                            { for score_rows.iter().map(|(label, val)| {
                                ...
                                html! {
                                    <div style="display:flex;align-items:center;gap:8px;">
                                        <span style="font-size:0.78rem;color:#8b949e;width:5.5rem;flex-shrink:0;">{*label}</span>
                                        ...
                                        <span style="font-size:0.75rem;color:#e6edf3;width:2.5rem;text-align:right;">{display}</span>
                                    </div>
                                }
                            }) }
                        </div>
...
pub(crate) fn layer_member_behavior(ctx: &InvestigationContext) -> Html {
        ...
        return html! {
            <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
                    <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"③ 成员行为模式"}</h4>
                </div>
                <p style="color:#8b949e;font-size:0.85rem;margin:0;">{"成员数据不可用"}</p>
            </div>
        };
    };
    ...
    let (risk_color, risk_label) = match member.risk_level_summary.to_ascii_lowercase().as_str() {
        s if s.contains("high") || s.contains("高") => ("#f85149", "高风险"),
        s if s.contains("medium") || s.contains("中") => ("#d29922", "中风险"),
        _ => ("#3fb950", "低风险"),
    };
    ...
    let total_f64   = member.total_claim_amount
        .as_f64()
        .filter(|&v| v > 0.0)
    ...
    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"③ 成员行为模式"}</h4>
                <span style={format!("background:#21262d;border:1px solid {risk_color};border-radius:12px;padding:2px 10px;font-size:0.75rem;font-weight:600;color:{risk_color};")}>
                    {risk_label}
                </span>
            </div>
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px;">
                <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px;text-align:center;">
                    <div style="font-size:1.3rem;font-weight:700;color:#e6edf3;">{claim_count}</div>
                    <div style="font-size:0.73rem;color:#8b949e;margin-top:2px;">{"累计理赔笔数"}</div>
                </div>
                <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px;text-align:center;">
                    <div style={format!("font-size:1.3rem;font-weight:700;color:{risk_color};")}>{high_risk}</div>
                    <div style="font-size:0.73rem;color:#8b949e;margin-top:2px;">{"高风险理赔"}</div>
                </div>
                <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px;text-align:center;">
                    <div style={format!("font-size:1.3rem;font-weight:700;color:{risk_color};")}>{format!("{pct_u32}%")}</div>
                    <div style="font-size:0.73rem;color:#8b949e;margin-top:2px;">{"高风险占比"}</div>
                </div>
            </div>
            ...
            <div style="margin-bottom:14px;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                    <span style="font-size:0.78rem;color:#8b949e;">{format!("高风险理赔占比 {pct_u32}%")}</span>
                </div>
            ...
            <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;margin-bottom:14px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="font-size:0.83rem;color:#8b949e;">{"累计理赔金额"}</span>
                    <span style={format!("font-size:0.9rem;font-weight:700;color:{};",
                        if total_f64 > 50000.0 { "#f85149" } else { "#e6edf3" })}>
                        {total_amount_str}
                    </span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-size:0.83rem;color:#8b949e;">{"本次理赔占历史总额比例"}</span>
                    <span style={format!("font-size:0.9rem;font-weight:700;color:{};",
                        if ratio_pct > 50 { "#d29922" } else { "#e6edf3" })}>
                        {format!("{ratio_pct}%")}
                    </span>
                </div>
            </div>
            ...
            { if !member.profile_summary.is_empty() {
                html! {
                    <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;margin-bottom:14px;">
                        <p style="margin:0 0 6px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"画像摘要"}</p>
                        <p style="margin:0;font-size:0.83rem;color:#e6edf3;line-height:1.5;">{&member.profile_summary}</p>
                    </div>
                }
            } else { html! {} } }
            ...
            <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                    <p style="margin:0;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"理赔时间窗口分布"}</p>
                    <span style="font-size:0.7rem;background:#21262d;color:#8b949e;border:1px solid #30363d;border-radius:4px;padding:1px 6px;">{"模拟数据"}</span>
                </div>
                <div style="display:flex;flex-direction:column;gap:6px;">
                    { for [("30天内", window_30), ("90天内", window_90), ("180天内", window_180)].iter().map(|(label, n)| {
                        ...
                        html! {
                            <div style="display:flex;align-items:center;gap:8px;">
                                <span style="font-size:0.78rem;color:#8b949e;width:4rem;flex-shrink:0;">{*label}</span>
                                <div style="flex:1;background:#21262d;border-radius:3px;height:8px;overflow:hidden;">
                                    <div style={format!("width:{bar_w}%;height:100%;background:#1f6feb;border-radius:3px;")}></div>
                                </div>
                                <span style="font-size:0.78rem;color:#e6edf3;width:3rem;text-align:right;">{format!("{n} 笔")}</span>
                            </div>
                        }
                    }) }
                </div>
                { if concentration_warning {
                    html! {
                        <div style="margin-top:10px;padding:6px 10px;background:#3d2000;border:1px solid #d29922;border-radius:4px;font-size:0.8rem;color:#d29922;">
                            {"⚠ 短期窗口内理赔集中度异常偏高"}
                        </div>
                    }
                } else { html! {} } }
...
pub(crate) fn layer_provider_analysis(ctx: &InvestigationContext) -> Html {
    ...
    let Some(prov) = provider else {
        return html! {
            <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
                    <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"④ Provider 风险分析"}</h4>
                </div>
    ...
    let has_p97_amount = prov.outlier_flags.iter().any(|f| f.contains("peer_amount_p97") || f.contains("amount_p97"));
    let has_p96_freq   = prov.outlier_flags.iter().any(|f| f.contains("peer_freq_p96") || f.contains("freq_p96"));

    let h             = simple_hash(&prov.provider_id);
    let my_amount     = 8000.0 + (h % 20000) as f64;
    let peer_amount   = if has_p97_amount { my_amount * 0.55 } else { my_amount * 0.85 };
    ...
    let (amount_rank_label, amount_rank_color) =
        if has_p97_amount { ("P97", "#f85149") } else { ("正常", "#3fb950") };
    let (freq_rank_label, freq_rank_color) =
        if has_p96_freq { ("P96", "#d29922") } else { ("正常", "#3fb950") };
    ...
    let member_prov_link_count = 1u32 + (simple_hash(&format!("{}{}", ctx.case.member_id, ctx.case.provider_id)) % 6) as u32;
    ...
    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"④ Provider 风险分析"}</h4>
                <span style={format!("background:#21262d;border:1px solid {score_color};border-radius:12px;padding:2px 10px;font-size:0.75rem;font-weight:600;color:{score_color};")}>
                    {score_label}
                </span>
                <span style="margin-left:auto;font-size:0.82rem;color:#8b949e;">{&prov.provider_id}</span>
            </div>

            // Risk score bar
            <div style="margin-bottom:14px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                    <span style="font-size:0.78rem;color:#8b949e;flex:1;">{"风险评分"}</span>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
                <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px;">
                    <div style="font-size:0.73rem;color:#8b949e;margin-bottom:4px;">{"专科"}</div>
                    <div style="font-size:0.88rem;color:#e6edf3;font-weight:600;">{specialty}</div>
                </div>
                <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px;">
                    <div style="font-size:0.73rem;color:#8b949e;margin-bottom:4px;">{"网络状态"}</div>
                    <div style="font-size:0.88rem;color:#e6edf3;font-weight:600;">{network_status}</div>
                </div>
            </div>
            ...
            <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;margin-bottom:14px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                    <p style="margin:0;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"同行对比"}</p>
                    <span style="font-size:0.7rem;background:#21262d;color:#8b949e;border:1px solid #30363d;border-radius:4px;padding:1px 6px;">{"模拟均值"}</span>
                </div>
                <div style="display:grid;grid-template-columns:7rem 1fr 1fr 3.5rem;gap:4px 10px;font-size:0.8rem;">
                    <span style="color:#8b949e;font-weight:600;padding-bottom:4px;border-bottom:1px solid #30363d;">{"指标"}</span>
                    <span style="color:#8b949e;font-weight:600;padding-bottom:4px;border-bottom:1px solid #30363d;">{"本 Provider"}</span>
                    <span style="color:#8b949e;font-weight:600;padding-bottom:4px;border-bottom:1px solid #30363d;">{"同行均值"}</span>
                    <span style="color:#8b949e;font-weight:600;padding-bottom:4px;border-bottom:1px solid #30363d;">{"排名"}</span>

                    <span style="color:#e6edf3;padding:3px 0;">{"理赔金额"}</span>
                    <span style={format!("color:{score_color};padding:3px 0;")}>{format!("¥{my_amount:.0}")}</span>
                    <span style="color:#8b949e;padding:3px 0;">{format!("¥{peer_amount:.0}")}</span>
                    <span style={format!("color:{amount_rank_color};padding:3px 0;font-weight:700;")}>{amount_rank_label}</span>

                    <span style="color:#e6edf3;padding:3px 0;">{"理赔频率"}</span>
                    <span style={format!("color:{};padding:3px 0;", if my_freq > peer_freq * 2 { "#f85149" } else { "#e6edf3" })}>{format!("{my_freq} 笔")}</span>
                    <span style="color:#8b949e;padding:3px 0;">{format!("{peer_freq} 笔")}</span>
                    <span style={format!("color:{freq_rank_color};padding:3px 0;font-weight:700;")}>{freq_rank_label}</span>

                    <span style="color:#e6edf3;padding:3px 0;">{"高费项目占比"}</span>
                    <span style={format!("color:{};padding:3px 0;", if my_high_item_pct > 50 { "#d29922" } else { "#e6edf3" })}>{format!("{my_high_item_pct}%")}</span>
                    <span style="color:#8b949e;padding:3px 0;">{format!("{peer_high_item_pct}%")}</span>
                    <span style="color:#8b949e;padding:3px 0;">{"—"}</span>
                </div>
            </div>
            ...
            { if !prov.graph_reasons.is_empty() {
                html! {
                    <div style="margin-bottom:14px;">
                        <p style="margin:0 0 8px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"风险依据"}</p>
                        <div style="display:flex;flex-direction:column;gap:5px;">
                            { for prov.graph_reasons.iter().map(|r| {
                                html! {
                                    <div style="background:#3d1a1a;border-left:3px solid #f85149;border-radius:0 4px 4px 0;padding:6px 10px;font-size:0.82rem;color:#e6edf3;">
                                        {r.as_str()}
                                    </div>
                                }
                            }) }
            ...
            { if !prov.outlier_flags.is_empty() {
                html! {
                    <div style="margin-bottom:14px;">
                        <p style="margin:0 0 8px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"异常标签"}</p>
                        <div style="display:flex;flex-wrap:wrap;gap:6px;">
            ...
            <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;margin-bottom:14px;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-size:0.83rem;color:#8b949e;">{"服务本成员次数"}</span>
                    <span style={format!("font-size:0.9rem;font-weight:700;color:{};",
                        if prov.claim_count > 3 { "#f85149" } else { "#e6edf3" })}>
                        {format!("{} 次", prov.claim_count)}
                    </span>
                </div>
                { if prov.claim_count > 3 {
                    html! {
                        <div style="margin-top:8px;padding:4px 8px;background:#3d1a1a;border:1px solid #f85149;border-radius:4px;font-size:0.78rem;color:#f85149;">
                            {"⚠ 服务频次异常偏高"}
                        </div>
                    }
                } else { html! {} } }
            ...
            <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-size:0.83rem;color:#8b949e;">{"该 Provider 与本成员理赔关联"}</span>
                    <span style={format!("font-size:0.9rem;font-weight:700;color:{};",
                        if member_prov_link_count > 3 { "#f85149" } else { "#e6edf3" })}>
...
pub(crate) fn layer_association_network(ctx: &InvestigationContext) -> Html {
    ...
    let shared_outlier_providers: Vec<&crate::types::ProviderRiskItem> = if let Some(prov) = provider {
        ctx.providers
            .iter()
    ...
    let has_confirmed_fwa_audit = ctx.audit_events.iter().any(|ev| {
        ev.event_type.contains("fwa") || ev.event_type.contains("confirmed")
            || ev.payload.get("fwa_confirmed").and_then(|v| v.as_bool()).unwrap_or(false)
    });
    ...
    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"⑤ 关联网络"}</h4>
                <span style="font-size:0.75rem;color:#8b949e;">{"是否存在群聚效应？"}</span>
            </div>

            // Mini SVG graph
            <div style="display:flex;justify-content:center;margin-bottom:14px;">
                <svg width="340" height="220"
                     style="background:#0d1117;border:1px solid #30363d;border-radius:8px;display:block;">

                    // Edges: Member → Provider
                    <line x1="70" y1="110" x2={format!("{}", 170 - prov_r as i32)}
                          y2="110" stroke="#58a6ff" stroke-width="1.5" />

                    // Edge label for Member → Provider
                    <text x="115" y="103" font-size="10" fill="#8b949e" text-anchor="middle">
                    ...
                    { for other_nodes.iter().map(|(_, ox, oy)| {
                        ...
                        html! {
                            <line x1={format!("{x1}")} y1={format!("{y1}")}
                                  ...
                                  stroke="#f85149" stroke-width="1.2"
                                  stroke-dasharray="4 3" />
                        }
                    }) }

                    // Member node (blue, fixed size 14)
                    <circle cx="70" cy="110" r="14" fill="#0d2d3a" stroke="#58a6ff" stroke-width="2" />
                    <text x="70" y="114" font-size="9" fill="#58a6ff" text-anchor="middle">{"M"}</text>
                    <text x="70" y="133" font-size="8" fill="#8b949e" text-anchor="middle">
                        { member_label.chars().take(8).collect::<String>() }
                    </text>

                    // Provider node (red, size from risk score)
                    <circle cx="170" cy="110" r={format!("{prov_r}")}
                            fill="#2d0a0a" stroke="#f85149" stroke-width="2" />
                    <text x="170" y="114" font-size="9" fill="#f85149" text-anchor="middle">{"P"}</text>
                    <text x="170" y={format!("{}", 110 + prov_r as i32 + 12)}
                          font-size="8" fill="#8b949e" text-anchor="middle">
                        ...
                        html! {
                            <>
                                <circle cx={format!("{ox}")} cy={format!("{oy}")} r="12"
                                        fill="#2d1a0a" stroke="#d29922" stroke-width="1.5" />
                                <text x={format!("{ox}")} y={format!("{}", *oy + 4)}
                                      font-size="9" fill="#d29922" text-anchor="middle">{"P"}</text>
                                <text x={format!("{ox}")} y={format!("{}", *oy + 20)}
                                      font-size="7" fill="#8b949e" text-anchor="middle">
                        ...
                        html! {
                            <>
                                <rect x="115" y="182" width="110" height="20"
                                      rx="4" fill="#3d1a1a" stroke="#f85149" stroke-width="1" />
                                <text x="170" y="196" font-size="9" fill="#f85149"
                                      text-anchor="middle">{"已确认 FWA"}</text>
                                <line x1="170" y1={format!("{}", 110 + prov_r as i32)}
                                      x2="170" y2="182"
                                      stroke="#f85149" stroke-width="1" stroke-dasharray="3 2" />
                            </>
                        }
                    } else { html! {} } }
                </svg>
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                { if confirmed_fwa_count > 0 {
                    html! {
                        <div style="padding:6px 10px;background:#3d1a1a;border:1px solid #f85149;border-radius:4px;font-size:0.82rem;color:#f85149;">
                ...
                { if shared_outlier_count > 0 {
                    html! {
                        <div style="padding:6px 10px;background:#3d2000;border:1px solid #d29922;border-radius:4px;font-size:0.82rem;color:#d29922;">
                ...
                { if confirmed_fwa_count == 0 && shared_outlier_count == 0 {
                    html! {
                        <div style="padding:6px 10px;background:#1b3a1f;border:1px solid #3fb950;border-radius:4px;font-size:0.82rem;color:#3fb950;">
                            {"✓ 未发现明显群聚效应"}
                        </div>
                    }
                } else { html! {} } }
...
pub(crate) fn layer_similar_cases(ctx: &InvestigationContext) -> Html {
    let mut sorted_cases: Vec<&crate::types::SimilarCaseItem> =
        ctx.similar_cases.iter().collect();
    sorted_cases.sort_by(|a, b| b.similarity_score.partial_cmp(&a.similarity_score).unwrap_or(std::cmp::Ordering::Equal));

    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"⑥ 相似已确认案例"}</h4>
            ...
            { if sorted_cases.is_empty() {
                html! {
                    <p style="color:#8b949e;font-size:0.85rem;margin:0 0 14px;">{"暂无相似历史案例"}</p>
                }
            } else {
                html! {
                        ...
                        { for sorted_cases.iter().map(|item| {
                            ...
                            let (outcome_bg, outcome_fg, outcome_text) = match item.final_outcome.as_deref() {
                                Some(o) if o.contains("confirmed_fwa") || o.contains("confirmed") =>
                                    ("#3d1a1a", "#f85149", "已确认 FWA"),
                                Some(o) if o.contains("false_positive") =>
                                    ("#1b3a1f", "#3fb950", "误报"),
                                Some(o) if o.contains("inconclusive") =>
                                    ("#2d2000", "#d29922", "不确定"),
                                Some(o) => ("#21262d", "#8b949e", o),
                                None    => ("#21262d", "#8b949e", "待定"),
                            };
                            let case_id_short = if item.case_id.len() > 16 {
                                format!("{}…", &item.case_id[..16])
                            } else {
                                item.case_id.clone()
                            };
                            ...
                            html! {
                                <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;">
                                    ...
                                    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;">
                                        <span style="font-size:0.82rem;color:#e6edf3;font-family:monospace;">
                                            {case_id_short}
                                        </span>
                                    ...
                                    <div style="margin-bottom:8px;">
                                        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px;">
                                            <span style="font-size:0.73rem;color:#8b949e;">{"相似度"}</span>
                                        ...
                                        html! {
                                            <div style="display:flex;flex-wrap:wrap;gap:4px;">
                                                { for item.tags.iter().map(|tag| {
                                                    html! {
                                                        <span style="background:#21262d;border:1px solid #30363d;border-radius:4px;padding:1px 6px;font-size:0.7rem;color:#8b949e;">
                                                            {tag.as_str()}
                                                        </span>
                                                    }
                                                }) }
            ...
            <div style="padding:6px 10px;background:#21262d;border:1px solid #30363d;border-radius:4px;font-size:0.75rem;color:#8b949e;">
...
pub(crate) fn layer_ai_summary(ctx: &InvestigationContext) -> Html {
    ...
    let Some(ev) = agent_event else {
        return html! {
            <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
                    <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"⑦ AI 调查摘要"}</h4>
                    <span style="font-size:0.72rem;background:#21262d;color:#8b949e;border:1px solid #30363d;border-radius:4px;padding:1px 8px;">
                        {"由 Agent 生成"}
                    </span>
                </div>
                <p style="color:#8b949e;font-size:0.85rem;margin:0 0 10px;">{"AI 调查包未生成"}</p>
                <p style="color:#8b949e;font-size:0.78rem;margin:0;">
                    {"当前案件尚未完成 Agent 自动调查，请等待系统生成或手动触发调查流程。"}
                </p>
                <div style="margin-top:14px;padding:6px 10px;background:#21262d;border:1px solid #30363d;border-radius:4px;font-size:0.75rem;color:#8b949e;">
        ...
        .map(|arr| {
            arr.iter()
                .filter_map(|item| {
                    item.as_str()
                        .map(str::to_string)
                        .or_else(|| item.get("item").and_then(|i| i.as_str()).map(str::to_string))
                        .or_else(|| item.get("description").and_then(|d| d.as_str()).map(str::to_string))
                })
                .collect()
        })
        .unwrap_or_default();
    ...
    let (es_bg, es_fg, es_label) = match es_status {
        Some(s) if s.contains("sufficient") || s.contains("充分") => ("#1b3a1f", "#3fb950", s),
        Some(s) if s.contains("insufficient") || s.contains("不足") => ("#3d1a1a", "#f85149", s),
        Some(s) if s.contains("partial") || s.contains("部分") => ("#3d2000", "#d29922", s),
        Some(s) => ("#21262d", "#8b949e", s),
        None    => ("#21262d", "#8b949e", "未知"),
    };
    ...
    html! {
        <div class="evidence-card" style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <h4 style="margin:0;color:#e6edf3;font-size:1rem;">{"⑦ AI 调查摘要"}</h4>
                <span style="font-size:0.72rem;background:#21262d;color:#58a6ff;border:1px solid #1f6feb;border-radius:4px;padding:1px 8px;">
                    {"由 Agent 生成"}
                </span>
            ...
            { if let Some(finding) = main_finding {
                html! {
                    <div style="background:#0d1117;border-left:4px solid #58a6ff;border-radius:0 6px 6px 0;padding:12px 14px;margin-bottom:14px;">
                        <p style="margin:0 0 6px;font-size:0.72rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"调查摘要"}</p>
                        <p style="margin:0;font-size:0.9rem;color:#e6edf3;line-height:1.6;font-style:italic;">
                            {finding}
                        </p>
                    </div>
                }
            ...
            { if !checklist.is_empty() {
                html! {
                    <div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:12px;margin-bottom:14px;">
                        <p style="margin:0 0 10px;font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;">{"调查清单"}</p>
                        <ol style="margin:0;padding-left:1.4rem;display:flex;flex-direction:column;gap:6px;">
                            { for checklist.iter().map(|item| {
                                html! {
                                    <li style="font-size:0.83rem;color:#e6edf3;line-height:1.5;">{item.as_str()}</li>
                                }
                            }) }
                        </ol>
                    </div>
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;">
                <span style="font-size:0.78rem;color:#8b949e;">{"证据充分性状态:"}</span>
