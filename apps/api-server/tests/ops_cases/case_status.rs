...
async fn updates_case_status_with_audit_trail() {
    ...
    let (status, body) = json_request(
        ...
        r#"{
          ...
          "actor_id": "siu-reviewer-2",
          "notes": "Investigation started with provider history review.",
          "evidence_refs": ["case_workflow:local://template/status.json"]
        }"#,
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    assert_eq!(body["code"], "INVALID_CASE_STATUS_EVIDENCE");
    ...
    let (status, body) = json_request(
        ...
        r#"{
          ...
          "actor_id": "siu-reviewer-2",
          "notes": "Investigation started with provider history review.",
          "evidence_refs": ["case_workflow:file://tmp/status.json"]
        }"#,
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
