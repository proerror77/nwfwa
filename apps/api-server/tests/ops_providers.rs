...
#[tokio::test]
async fn provider_write_paths_reject_template_source_report_uri() {
    let app = build_app(test_config_with_provider_actors()).unwrap();
    for (path, payload, code) in [
        ...
        payload["source_report_uri"] =
            serde_json::json!("local://template/provider/source_report.json");
...
#[tokio::test]
async fn provider_profile_window_rollup_rejects_template_record_evidence_refs() {
    let app = build_app(test_config_with_provider_actors()).unwrap();
    let mut payload: serde_json::Value =
        serde_json::from_str(provider_profile_window_rollup_payload()).unwrap();
    payload["provider_profiles"][0]["evidence_refs"] =
        serde_json::json!(["claims:local://template/provider/claim-snapshot.json"]);
