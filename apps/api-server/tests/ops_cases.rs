...
async fn creates_lead_from_high_risk_scoring_and_triages_to_case() {
    ...
    let (status, body) = json_request(
        ...
        r#"{
          ...
          "priority": "high",
          "notes": "Open investigation from high-risk FWA lead.",
          "evidence_refs": ["triage_decisions:local://template/open-case.json"]
        }"#,
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    assert_eq!(body["code"], "INVALID_TRIAGE_REVIEW_EVIDENCE");
    ...
    let (status, body) = json_request(
        ...
        r#"{
          ...
          "priority": "high",
          "notes": "Open investigation from high-risk FWA lead.",
          "evidence_refs": ["triage_decisions:file://tmp/open-case.json"]
        }"#,
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    let (status, body) = json_request(
        ...
        r#"{
          ...
          "priority": "high",
          "notes": "Open investigation from high-risk FWA lead.",
          "evidence_refs": ["triage_decisions:http://localhost:8080/open-case.json"]
        }"#,
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
