...
The package includes API, web console, ML service, worker CronJobs,
PostgreSQL, S3-compatible object storage, ClickHouse, database migration,
Ingress, HPA, PDB, and NetworkPolicy manifests. It intentionally does not
include customer data or a demo seed job.

Apply only from an approved production Kubernetes context with real image tags,
TLS, and `NWFWA_PRODUCTION_SECRET_FILE`.
"""
...
def copy_and_patch_manifests(
    ...
    for resource in ["ingress.yaml", "hpa.yaml", "pdb.yaml", "networkpolicy.yaml"]:
...
def build_package(
    ...
    manifests = copy_and_patch_manifests(output_dir, namespace, images, host, tls_secret)
...
kubectl -n "$namespace" get networkpolicy default-deny-ingress allow-same-namespace allow-ingress-controller-to-web-and-api
