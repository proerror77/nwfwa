...
include `canonical_claim_context_trace` in the event payload. API call and
webhook event records expose `customer_scope_id` from authenticated API key
configuration so TPA traffic and delivery attempts can be reviewed by
