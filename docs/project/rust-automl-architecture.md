...
- `run-mlops-monitoring-plan`: execute the scheduled monitoring plan as a Rust
  ...
  label, or rule state.
- `run-scheduled-mlops-monitoring`: generate the scheduled monitoring plan and
  runtime reports in one deployable Rust worker command for CronJob or external
  scheduler use.
...
- 98% for Auto MLOps: worker can build feature-set manifests, create
