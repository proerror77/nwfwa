...
| Model operations and MLOps pipeline | Implemented with customer validation boundary | `apps/api-server/src/routes/ops_models.rs`, `crates/fwa-ml-runtime`, `apps/worker`, `docs/project/ml-pipeline-runbook.md` | Real labels, live shadow traffic, customer holdout, production drift evidence, PSI actioning, rule hit-rate trending, and calibrated probability evidence. |
...
| Knowledge, agent, and AI evidence foundation | Staging proof with control-plane gaps | `apps/api-server/src/routes/knowledge.rs`, `apps/api-server/src/routes/ops_evidence.rs`, `scripts/ops/build_ai_evidence_foundation.py` | Customer OCR, embedding/vector store, retrieval ranking, masking, retention execution, runtime agent registry, investigation entity, PHI field enforcement, and kill-switch behavior. |
...
| --- | --- | --- |
| P0 | Proxy/placeholder labeling for L1 peer benchmark, L5 clinical compatibility, and L3 anomaly baseline | PRD requirement; implementation must keep clear evidence that these are not production-confirmed signals. |
| P0 | Routing policy lifecycle permission checks | Must be verified against code before claiming complete. |
| P0 | Inbox validation/audit serialization failures must fail loudly | Must be verified against code before claiming complete. |
| P0 | Runtime scorer lock-poison recovery | Must be verified against code before claiming complete. |
| P1 | Missing-data-aware score reweighting and revised confidence scoring | PRD requirement; production readiness depends on implementation and tests. |
| P1 | Worker-owned OIG/SAM sync and provider 30/90/365 profile windows | Required data-pipeline ownership; staging/demo inputs are not enough. |
| P1 | PSI threshold actioning and rule hit-rate runtime trending | Monitoring must create review tasks or alerts, not just reports. |
| P1 | Field-path PII/PHI masking for canonical contexts and evidence responses | Required for HIPAA-safe API and audit surfaces. |
| P2 | Billing ring, temporal co-billing, referral entropy, peer percentile, and episode aggregation worker rollups | Required before broad FWA scheme coverage can be called production-complete. |
| P2 | Agent registry, investigations entity, PHI field allowlists, and populated `phi_fields_accessed` | Required for agentic investigation governance. |
| P3 | Agent kill-switch, specialist-agent orchestration, six-year audit retention execution, and statistical L3 anomaly upgrade | Medium-term production hardening. |
