...
| Model operations and MLOps pipeline | Implemented with customer validation boundary | `apps/api-server/src/routes/ops_models.rs`, `crates/fwa-ml-runtime`, `apps/worker`, `docs/project/ml-pipeline-runbook.md`; PSI actioning, rule hit-rate trending, sanctions sync contract, and provider profile rollups verified in `b6d56ae`, `b0f00fa`, and `d17f3bd` | Real labels, live shadow traffic, customer holdout, production drift evidence, and calibrated probability evidence. |
...
| Knowledge, agent, and AI evidence foundation | Staging proof with partial control-plane contract | `apps/api-server/src/routes/knowledge.rs`, `apps/api-server/src/routes/ops_evidence/mod.rs`, `scripts/ops/build_ai_evidence_foundation.py`; agent registry and investigation grouping contract verified in `fee9ea9` | Customer OCR, embedding/vector store, retrieval ranking, masking, retention execution, runtime registry allowlist enforcement, and kill-switch behavior. |
...
| P2 | Agent registry, investigations entity, PHI field allowlists, and populated `phi_fields_accessed` | Partially implemented and verified in `fee9ea9`: additive `agent_registry` and `investigations` schema contract, stable investigation ids for agent audit events, exposed agent identity/investigation fields in run logs, and non-empty PHI field audit records. Dynamic runtime allowlist enforcement before each tool boundary remains open. |
