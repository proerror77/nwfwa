...
| P1 | Worker-owned OIG/SAM sync and provider 30/90/365 profile windows | OIG/SAM sanctions sync is implemented as a local worker contract in `b6d56ae` and now has a permission-gated submit/write path that persists provider sanctions without changing scoring policy or assigning fraud labels; verified by `cargo test --locked -p api-server --test ops_providers sanctions` and `cargo test --locked -p worker sanctions`. Provider 30/90/365 profile windows remain implemented as a local worker contract in `b0f00fa`, verified by `cargo test --locked -p worker provider_profile` and `cargo check --locked -p worker`. Live external OIG/SAM fetch, production scheduling, and provider-profile DB write paths remain customer-environment work. |
