...
Persisted OIG/SAM sanctions can populate claim-time provider sanctions scoring
inputs even when the request has no inline `provider_profile`.
...
| P1 | Worker-owned OIG/SAM sync and provider 30/90/365 profile windows | OIG/SAM sanctions sync is implemented as a local worker contract in `b6d56ae` and now has a permission-gated submit/write path that persists provider sanctions without changing scoring policy or assigning fraud labels; claims scoring can load persisted provider sanctions and trigger sanctions-only provider review when no inline profile is supplied, covered by `cargo test --locked -p api-server --test claims_score persisted_provider_sanctions`; submit/write behavior remains verified by `cargo test --locked -p api-server --test ops_providers sanctions` and `cargo test --locked -p worker sanctions`. Provider 30/90/365 profile windows are implemented as a local worker contract in `b0f00fa` and now have a permission-gated submit/write path that persists provider profile windows without changing scoring policy or assigning fraud labels; claims scoring can load the latest persisted provider profile window rollup when no inline profile is supplied, covered by `cargo test --locked -p api-server --test claims_score persisted_provider_profile_rollup`; submit/write behavior remains verified by `cargo test --locked -p api-server --test ops_providers profile_window`, `cargo test --locked -p worker provider_profile`, and `cargo check --locked -p worker`. Live external OIG/SAM fetch, production scheduling, and customer claim-history validation remain customer-environment work. |
...
| P2 | Worker data pipeline schedule, readiness, and execution evidence contracts | Implemented as worker artifacts plus audit write paths: `build-worker-data-pipeline-plan` defines daily/monthly cadence, artifact roots, command order, dependency ordering, API submit endpoints, idempotency key shape, readiness gates, and a no-adjudication side-effect boundary across sanctions, provider profile, graph signals, peer benchmarks, episodes, clinical references, unbundling, scoring context materialization, and probability calibration. `build-worker-data-pipeline-readiness-report` validates customer-approved artifact URIs, minimum row counts, data-quality status, external OIG/SAM fetch configuration, and evidence refs before scheduled writes; `/api/v1/ops/worker-data-pipeline-readiness` persists that prerequisite evidence and records audit lineage without fetching external data or submitting artifacts. `build-worker-data-pipeline-run-status-template` generates the customer scheduler run-status input contract with an explicit readiness report URI and pending job statuses. `build-worker-data-pipeline-execution-report` converts a customer scheduler run-status artifact into per-job execution status, readiness-gate status, readiness evidence refs, and operations review tasks, and `/api/v1/ops/worker-data-pipeline-executions` persists that scheduler evidence and records audit lineage without scoring claims, assigning labels, denying claims, activating models, or changing routing policy. Missing or blocked readiness evidence now remains visible in execution evidence instead of silently bypassing the customer-data readiness gate. Live scheduler deployment, credentials, external OIG/SAM fetch, and customer-data validation remain open. |
