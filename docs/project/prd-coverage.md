...
| P2 | Clinical compatibility feature input path | Implemented in `a63e45f`: `fwa-features` accepts `ClinicalCompatibilityFeatureContext` so governed ICD-10/CPT compatibility scores can replace the heuristic `diagnosis_procedure_match_score` and be marked non-proxy. Building the governed reference dataset and worker rollup remains open. |
