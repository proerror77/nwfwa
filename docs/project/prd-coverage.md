...
| Core scoring, rules, and review modes | Implemented with baseline and customer-validation boundary | `crates/fwa-scoring`, `crates/fwa-rules`, `apps/api-server/src/routes/ops_rules.rs`, demo seed/smoke, `docs/product/fwa-risk-operations-prd.md`; missing-data reweighting and broader confidence verified in `eccfc06` and `a2e7015`; clinical compatibility reference-score input path verified in `a63e45f`; clinical compatibility reference worker contract verified in `3aaa58d` | Customer-specific approved rule lists, production threshold refs, appeal/override routes, customer-approved ICD-10/CPT or medical-policy reference data, and live routing-impact evidence are required before production adjudication. |
