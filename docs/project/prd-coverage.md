...
| Knowledge, agent, and AI evidence foundation | Staging proof with control-plane gaps | `apps/api-server/src/routes/knowledge.rs`, `apps/api-server/src/routes/ops_evidence/mod.rs`, `scripts/ops/build_ai_evidence_foundation.py` | Customer OCR, embedding/vector store, retrieval ranking, masking, retention execution, runtime agent registry, investigation entity, PHI field enforcement, and kill-switch behavior. |
