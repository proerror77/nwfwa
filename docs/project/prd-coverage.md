...
| P2 | Episode utilization scoring features | Implemented in `e2b4bf6`: `fwa-features` accepts worker-owned member-provider revisit counts, duplicate-claim similarity, procedure-frequency peer percentile, and unbundling candidate counts, and `fwa-scoring` consumes them in L5 with bounded risk contribution. Production worker-to-online scoring plumbing remains open. |
